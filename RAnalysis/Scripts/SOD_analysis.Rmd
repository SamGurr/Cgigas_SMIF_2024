...
exp.metadata <- read.csv('Data/experiment_metadata/SMIF_Sample_Info_SG_ParallelAnalyses.csv', sep = ",", header=T) %>% 
                      dplyr::select(c('Set', 'Tube.ID', 'Assay..SOD.CAT.')) %>%  # call target columns 
                      dplyr::filter(Assay..SOD.CAT. %in% 'SOD') %>%  # redundant sample ids for each assay, arbitrarily choose SOD to get unique IDs
                      dplyr::mutate(Time = gsub("_.*", "", Tube.ID), # make column Time by calling everying before the first _
                      ...
                      dplyr::select(ID, Time, Treatment)
...
# plate 1 
SOD_plate1 <- Spec_to_table('Data/raw_data/noaa/superoxide_dismutase/20250226_sod_plate1.txt', 1, 20250226) %>%  
                                  dplyr::rename(SOD_Absorbance_450nm=absorbance_nm) 
SOD_plate1_dfo <- Spec_to_table_dfo('Data/raw_data/dfo/superoxide_dismutase/SOD_plate1.csv', 1) %>%  
                                  dplyr::rename(SOD_Absorbance_450nm=absorbance_nm) 

# plate 2 
SOD_plate2 <- Spec_to_table('Data/raw_data/noaa/superoxide_dismutase/20250226_sod_plate2.txt', 2, 20250226) %>%  
                                  dplyr::rename(SOD_Absorbance_450nm=absorbance_nm) 
SOD_plate2_dfo <- Spec_to_table_dfo('Data/raw_data/dfo/superoxide_dismutase/SOD_plate2.csv', 2) %>%  
                                  dplyr::rename(SOD_Absorbance_450nm=absorbance_nm) 
# master 
SOD_raw     <- rbind(SOD_plate1, SOD_plate2)
SOD_raw_dfo <- rbind(SOD_plate1_dfo, SOD_plate2_dfo)
```

```{r load Total protein correction data for catalase}

# plate 1 
totalprotein_plate1 <- Spec_to_table('Data/raw_data/noaa/superoxide_dismutase/20250227_total_protein_sod_plate1.txt', 1, 20250227) %>%  
                                  dplyr::rename(TP_Absorbance_562nm=absorbance_nm) 
totalprotein_plate1_dfo <- Spec_to_table_dfo('Data/raw_data/dfo/superoxide_dismutase/SOD_Protein_plate1.csv', 1) %>%  
                                  dplyr::rename(TP_Absorbance_562nm=absorbance_nm) 

# plate 2
totalprotein_plate2 <- Spec_to_table('Data/raw_data/noaa/superoxide_dismutase/20250227_total_protein_sod_plate2.txt', 2, 20250227) %>%  
                                  dplyr::rename(TP_Absorbance_562nm=absorbance_nm) 
totalprotein_plate2_dfo <- Spec_to_table_dfo('Data/raw_data/dfo/superoxide_dismutase/SOD_Protein_plate2.csv', 2) %>%  
                                  dplyr::rename(TP_Absorbance_562nm=absorbance_nm) 

# master 
totalprotein_raw     <- rbind(totalprotein_plate1, totalprotein_plate2)
totalprotein_raw_dfo <- rbind(totalprotein_plate1_dfo, totalprotein_plate2_dfo)
...
metadata     <- read.csv('Data/raw_data/noaa/plate_metadata.csv', sep = ",", header=T) 
...
# Plate 1 
meta.SOD_plate1_dfo <- metadata_dfo %>% 
                        dplyr::filter(Assay.Type %in% "Superoxide dismutase" & Plate.Number == 1) %>% 
                        dplyr::rename(Plate = Plate.Number,
                                      well  = Well.ID)
# Plate 2
meta.SOD_plate2_dfo <- metadata_dfo %>% 
                        dplyr::filter(Assay.Type %in% "Superoxide dismutase" & Plate.Number == 2) %>% 
                        dplyr::rename(Plate = Plate.Number,
                                      well  = Well.ID)
...
# SDO
# Plate 1 
SOD_plate1_master_dfo <- merge(SOD_plate1_dfo, meta.SOD_plate1_dfo, 
                                by = c('Plate', 'well')) 

nrow(meta.SOD_plate1_dfo) == nrow(SOD_plate1_master_dfo) # must be TRUE
SOD_plate1_master_dfo <- subset(SOD_plate1_master_dfo, !is.na(Sample.Type)) # now 96  total rows with NAs omitted (not sample, blank nor stnadard)


# Plate 2
SOD_plate2_master_dfo <- merge(SOD_plate2_dfo, meta.SOD_plate2_dfo, 
                                by = c('Plate', 'well')) 

nrow(meta.SOD_plate2_dfo) == nrow(SOD_plate2_master_dfo) # must be TRUE
SOD_plate2_master_dfo <- subset(SOD_plate2_master_dfo, !is.na(Sample.Type)) # now 52  total rows with NAs omitted (not sample, blank nor stnadard)
...
* Generated curve with standards
  - calculate the mean blank standard as 0 SOD_UmL 
  - correct standards for the blank absorbance 
  
  
* Calcaulate the sample U/mL 
  - correct absorbance for blank 
  - use quadratic equation from standard curve to calculate U/mL SOD


* STEP 1: Calculate blank absorbance

```{r NOAA SOD blank absorbnace}
...
Plate1_standards_meanSE_dfo <- SOD_plate1.standards_dfo %>% 
                                    ...
                                    dplyr::summarise(meanAbs = mean(Absorbance_450nm_raw),
                                                     sdAbs   = sd(Absorbance_450nm_raw),
                                                     n = n())
Plate1_standards_meanSE_dfo <- Plate1_standards_meanSE_dfo %>% filter(!SOD_UmL %in% c(4,2,1))
...
# Parameters:
#            Estimate Std. Error t value Pr(>|t|)  
# yf          0.05540    0.03595   1.541   0.2632  
# y0         64.70310  143.85998   0.450   0.6969  
# log_alpha   3.66850    0.44054   8.327   0.0141 *
...
Plate2_standards_meanSE_dfo <- SOD_plate2.standards_dfo %>% 
                                    ...
                                    dplyr::summarise(meanAbs = mean(Absorbance_450nm_raw),
                                                     sdAbs   = sd(Absorbance_450nm_raw),
                                                     n = n())
Plate2_standards_meanSE_dfo <- Plate2_standards_meanSE_dfo %>% filter(!SOD_UmL %in% c(4,2,1))
...
# Parameters:
#           Estimate Std. Error t value Pr(>|t|)   
# yf         0.01792    0.02488   0.720  0.54611   
# y0         2.35294    0.73154   3.216  0.08458 . 
# log_alpha  2.97723    0.19431  15.322  0.00423 **
...
## Calculate superoxide dismutase
...
* Steps: 
  ...
  - merge means total protein with the superoxide dismutase absorbance data
  
    - quick sanity check to ensure the merge was successful
    
  - calculate superoxide dismutase using the standard equations from the chunk above
...
```{r NOAA merge total protein wit superoxide dismutase samples}
...
```{r DFO merge total protein wit superoxide dismutase samples}
# now bind together all the SOD samples
...
SOD_Uml_ngprotein_final_dfo <- SOD_all_protein_dfo %>% # row bind, merge both together
                                          ...
                                          dplyr::mutate(
                                            SOD_UmL = # new column
                                              case_when( # dependencies of the new column  when...
                                                Plate %in% 1 ~ SSasymp(SOD_Absorbance_450nm, 0.06, 64.70, 3.67),
                                                Plate %in% 2 ~ SSasymp(SOD_Absorbance_450nm, 0.02, 2.35, 2.98) #13.78, -1.40, 0.96)
                                                #Plate %in% 2 ~ SSasymp(SOD_Absorbance_450nm,  6.96, -17.28, 3.45)
                                                #Plate %in% 1 ~ 0.82 + (-0.34 - 0.82) / (1 + (Cat_Absorbance_560nm / 24.77)^0.01), # column plate is 1, EQ,
                                                #Plate %in% 2 ~ 1.66 + (-0.22 - 1.66) / (1 + (Cat_Absorbance_560nm / 5.63)^0.20) # column plate is 2, EQ,
                                              ),
                                            SOD_UmL_ng_protein = 
                                              (SOD_UmL*Dilution.factor) / (TotalProtein_ug /1000),
                                            Treatment = 
                                              case_when(
                                                grepl("BL", Sample_ID) ~ "Baseline", 
                                                grepl("A1|A2|A3|B1|B2|B3", Sample_ID) ~ "Control",
...
pdf(paste("Output/dfo/superoxide_dismutase/plots/SODUmL_standard_curves_with_data.pdf", sep =''), 
    width=10, 
    height=6)
