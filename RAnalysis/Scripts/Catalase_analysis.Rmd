...
* STEP 2: Generate standard curve (correct for blank background abs first!)
  ...
  - plate1: y = 0.82 + (−0.34 − 0.82) / (1 + (x / 24,77)^0.01); x = Abs at 500 um, y = catalase U mL
  - plate2: y = 1.33 + (−0.03 − 1.33) / (1 + (x / 1.09)^0.86); x = Abs at 500 um, y = catalase U mL
...
Catalase_Uml_ngprotein_final <- catalase_all_protein %>% # row bind, merge both together
                                          dplyr::mutate(
                                           ...
                                           dplyr::select(Sample_ID, Treatment, Time, Catalase_UmL_ng_protein)
